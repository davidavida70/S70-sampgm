...
{
    if(!IsPlayerConnected(playerid)) return SendClientMessage(playerid, limao, "Voce n�o est� conectado");
    ShowPlayerDialog(playerid, lradios, DIALOG_STYLE_LIST, "Lista de r�dios",
        ...
        case lradios:
        {
            if(response)
            {
                switch(listitem)
                {
                    ...
                    {
                        PlayAudioStreamForPlayer(playerid, "http://stream.zeno.fm/ru0zuqf8l96vv");
         	            SendClientMessage(playerid, 0x4D00C1FF, "Tocando agora: R�dio do Davidavida70");
                    }
                    case 1:
                    {
                        PlayAudioStreamForPlayer(playerid, "http://stream.zeno.fm/5fhrkkylcicvv");
